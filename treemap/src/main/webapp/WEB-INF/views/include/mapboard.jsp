...
<%@ taglib uri="http://www.springframework.org/security/tags"
	prefix="sec"%>
<link
	href="<%=request.getContextPath()%>/resources/css/mapboardNav.css"
...
<body>
	<div id="boardNav" class="boardNav">
		<div class="category">
			<div onclick="reloadMapList();"
				style="margin-left: 30px; width: 70px; height: 100%; cursor: pointer; display: flex; justify-content: center; align-items: center;">
				<span style="font-size: 13px; color: white;">전체보기</span>
			</div>
			<ul>

				<c:if test="${categoryPage.startNum>3}">
					<li class="catName" style="color: white;"
						onclick="reloadMapListKeyword(1,${categoryPage.num-1},'${page.searchType}','${page.keyword}')"><</li>
				</c:if>
				<c:forEach items="${categoryName}" var="list" varStatus="status">

					<c:if test="${keyword != list.catName}">
						<li class="catName"
							onclick="reloadMapListKeyword(1,${categoryPage.num},'catName','${list.catName}')">${list.catName}</li>
					</c:if>
					<c:if test="${keyword == list.catName}">
				...
				<c:if
					test="${categorylength > categoryPage.startNum+categoryPage.endNum}">
					<li class="catName" style="color: white;"
						onclick="reloadMapListKeyword(1,${categoryPage.num+1},'${page.searchType}','${page.keyword}')">></li>
				</c:if>
			</ul>
		...
		<div id="mapboardListBox" class="mapboardListBox">
			<div class="mapboardListSearch">
				...
				<button class="SearchBtn"
					onclick="reloadMapListKeyword(1,1,'adrName','')">
					<img class="searchImg"
						src="<%=request.getContextPath()%>/resources/imgs/searchgray.png">
				</button>
			</div>
			<c:choose>
				<c:when test="${address.detail}">
					<div class="boardDetailwapper">
						<div class="boardDetail">
							...
							<div class="MapChange"
								style="background-color: rgb(35, 140, 250)">
								<button type="button" class="modifyBtn" id="findRoad"
									style="background-color: blue"
									onclick="openKaKao('${address.placeName}','${address.lat}','${address.lng}')">길찾기</button>
								<c:if test="${address.placeUrl ne 'undefined'}">
									<c:set var="homepage" value="true" />
									<button type="button" class="modifyBtn" id="homePage"
										onclick="openUrl('${address.placeUrl}')">홈페이지</button>
								</c:if>
								<button type="button" class="modifyBtn" id="detailModity"
									onclick="modifyModel()">수정</button>
								<button type="button" class="modifyBtn" id="detailDelete"
									onclick="deleteMapboard(${address.adrNo},${category.catNo})">삭제</button>
							</div>
						</div>
						<div style="margin-top: 20px;">
							<button type="button" class="modifyBtn"
								style='background-color: rgb(35, 140, 250);'
								onclick="reloadMapCategoryList()">돌아가기</button>
						</div>
					</div>
				</c:when>
				<c:otherwise>
					<c:if test="${empty mapBoardList }">
						<div class="listEmpty">
							<p class="emptySpan">등록된 지도가 없습니다.</p>
							<p class="emptySpan">지도를 클릭해 나만의 지도를</p>
							<p class="emptySpan">등록해보세요!</p>
						</div>
					</c:if>
					...
					<div class="pageBox">
						<c:if test="${page.prev}">
							<button class="pageNumBtn"
								onclick="reloadMapListKeyword(${page.startPageNum - 1},${categoryPage.num},'${page.searchType}','${page.keyword}')"><</button>
						</c:if>
						...
						<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}"
							...
							<span> <c:if test="${select != num}">
									<button class="pageNumBtn"
										onclick="reloadMapListKeyword(${num},${categoryPage.num},'${page.searchType}','${page.keyword}')">${num}</button>
								</c:if> <c:if test="${select == num}">
									<button class="pageNumBtn"
										style="background-color: rgb(0, 50, 150); color: white;">${num}</button>
						...
						<c:if test="${page.next}">
							<button class="pageNumBtn"
								onclick="reloadMapListKeyword(${page.endPageNum + 1},${categoryPage.num},'${page.searchType}','${page.keyword}')">></button>
						</c:if>
					</div>
				</c:otherwise>
			</c:choose>
		...
		<div class="modal-content">
			...
			<div class="modalBox">
				<label class="modalLabel">카테고리</label> <input class="modalInput"
					id="Mcategory" type="text" value="${category.catName}" />
			</div>
			<div class="modalBox">
				<label class="modalLabel">제목</label><input class="modalInput"
					id="Maddressname" type="text" value="${address.adrName}" />
			</div>
			<div class="modalBox">
				<label class="modalLabel">메모</label>
				<textarea class="modalmemo" id="Mmemo">${address.memo}</textarea>
			</div>
			<div class="modalBox">
				<label class="modalLabel">마커 선택</label>
				<div class="markerBtnDiv">
					<div class="markerBtnDiv2">
						<button class="markerBtn" id="Mdefault"
							onclick="markerSelect('Mdefault','<%=request.getContextPath()%>/resources/imgs/default.png')">
							<img style="width: 50px;"
								src="<%=request.getContextPath()%>/resources/imgs/default.png">
						</button>
						<button class="markerBtn" id="Mfood"
							onclick="markerSelect('Mfood','<%=request.getContextPath()%>/resources/imgs/food.png')">
							<img style="width: 50px;"
								src="<%=request.getContextPath()%>/resources/imgs/food.png">
						</button>
						<button class="markerBtn" id="Mbank"
							onclick="markerSelect('Mbank','<%=request.getContextPath()%>/resources/imgs/bank.png')">
							<img style="width: 50px;"
								src="<%=request.getContextPath()%>/resources/imgs/bank.png">
						</button>
					</div>
					<div class="markerBtnDiv2">
						<button class="markerBtn" id="Mmart"
							onclick="markerSelect('Mmart','<%=request.getContextPath()%>/resources/imgs/mart.png')">
							<img style="width: 50px;"
								src="<%=request.getContextPath()%>/resources/imgs/mart.png">
						</button>
						<button class="markerBtn" id="Mhome"
							onclick="markerSelect('Mhome','<%=request.getContextPath()%>/resources/imgs/home.png')">
							<img style="width: 50px;"
								src="<%=request.getContextPath()%>/resources/imgs/home.png">
						</button>
						<button class="markerBtn" id="Mhospital"
							onclick="markerSelect('Mhospital','<%=request.getContextPath()%>/resources/imgs/hospital.png')">
							<img style="width: 50px;"
			...
			<div
				...
				<div style="width: 40%">
					<button class="fBtn"
						onClick="modifyFavorites(${address.adrNo},${category.catNo});">
